...
class ShoreCefRouter(
    ...
    private fun handleIntent(request: JsToKotlinRequest): KotlinToJsResponse =
        when (request.method) {
            ...
            "executeRawQuery" -> executeRawQuery(request.params["query"].toString())
    ...
    private fun executeRawQuery(query: String): KotlinToJsResponse {
        println("executeRawQuery: $query")
        val statement = connection.prepareStatement(query)
        val hasResultSet = statement.execute()
        if (hasResultSet) {
            val resultSet = statement.resultSet
            return KotlinToJsResponse(true, resultSet.toString())
        } else {
